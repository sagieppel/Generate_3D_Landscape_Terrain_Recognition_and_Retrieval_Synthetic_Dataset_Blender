...
#    if np.random.rand()<0.5:
    bpy.data.worlds["World"].node_tree.nodes["Background.001"].inputs[1].default_value = 1+np.random.rand()*3.5
#    else:     
...
# Set camera position (so it will look at the vessel)

#################################################################
def SetCameraPos(name,VesWidth,VesHeight):
     print("Randomly set camera position")
     MinDist=np.max([VesWidth,VesHeight])
